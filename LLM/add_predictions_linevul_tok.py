...
def main(args):
    ...
    for dataset_path in DATASETS:
        ...
        def add_predictions(sample):
            input_ids = tokenizer(
                list(map(tokenize, sample["processed_func"])),
                truncation=True,
        ...
        with torch.cuda.amp.autocast():
            data = data.map(add_predictions, batched=True, batch_size=8, remove_columns=prev_columns)
        print(f"Eval for {dataset_path}")
        model_type = args.checkpoint.lstrip("./").lstrip("/").split("/")[0]
        data.save_to_disk(os.path.join("report/prediction/linevul", model_type, dataset_path))
